#############################################################################################
# This Feature file will test the the E2E functionality of payment for JAL.
...
Feature: Validate Payment JAL


  Scenario Outline: Validate a successful MOBILE purchase to JAL with a VISA
    When I generate a MOBILE GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
  ...
  Scenario Outline: Validate a successful LAPTOP purchase to JAL with a VISA
    When I generate a LAPTOP GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  Scenario Outline: Validate a successful LAPTOP purchase to JAL with a VISA
   ...
   @Positive
  Scenario Outline: Validate a successful MOBILE purchase to JAL with a SAVED VISA card
    When I generate a MOBILE GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
   ...
   @Positive
  Scenario Outline: Validate a successful LAPTOP purchase to JAL with a SAVED VISA card
    When I generate a LAPTOP GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Positive
  Scenario Outline: Validate a successful purchase to JAL with a VISA in all available locales and currencies
    When I generate a MOBILE GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
    ...
    Examples:
      ...
      | en_US  | JPY      | JLSPLA0002  |
      | ja_JP  | JPY      | JLSPLA0002  |

  @Negative
  Scenario Outline: Validate a successful purchase to JAL with an invalid card passes
    When I generate a MOBILE GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a successful purchase to JAL with an invalid user name
    When I generate a MOBILE GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a successful purchase to JAL with an invalid uxdid
    When I generate a MOBILE GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a successful purchase to JAL with an invalid zip code format
    When I generate a MOBILE GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  Scenario Outline: Validate a valid product with an invalid price
    When I generate a <sessionType> GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a successful purchase to JAL with an invalid product code
    When I generate a MOBILE GBP SessionId with airlineCode = "JAL", tailNumber = "0JAL1", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  Scenario Outline: Validate a valid product from a different airline
    ...
    When I send an APIDecorator POST request to "/v2/captcha/validate/" with the generated body
    Then the response status should be "200"
    Given I generate a process order model with card type VISA for productCode = "NUSAAA0001", price = "0", user = "%{userId}", uxdId = "%{uxdid}", locale = "ja_JP", and currency = "JPY"
    When I send an APIDecorator POST request to "/v2/processorder/" with the generated body
    Then the response status should be "400"
    And the JSON response should include the following:
      | trackingId |
