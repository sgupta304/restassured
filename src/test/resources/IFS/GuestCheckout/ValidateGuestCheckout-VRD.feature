#############################################################################################
# This Feature file will test the the E2E functionality of Guest Checkout for VRD
...
Feature: Validate Guest Checkout VRD


  Scenario Outline: Validate a successful VRD guest checkout with a VISA card for all applicable productCodes
    ...
    Examples:
      ...
      | MOBILE      | VRD         | 0VRD2      | VXSPDM0000  | en_US  | USD      |
      | LAPTOP      | VRD         | 0VRD2      | VXSPDF0003  | en_US  | USD      |
  ...
  Scenario Outline: Validate a successful VRD guest checkout with a VISA card for all applicable productCodes
    ...
    Examples:
      | sessionType | airlineCode | tailNumber | productCode | locale | currency |
      | MOBILE      | VRD         | 0VRD2      | VXSPDM0000  | en_US  | USD      |
      | MOBILE      | VRD         | 0VRD2      | VXTPDM0030  | en_US  | USD      |
      | LAPTOP      | VRD         | 0VRD2      | VXSPDF0003  | en_US  | USD      |
      | LAPTOP      | VRD         | 0VRD2      | VXTPDL0030  | en_US  | USD      |

  @Positive
  Scenario Outline: Validate a successful VRD guest checkout with an MC card for all applicable productCodes
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Positive
  Scenario Outline: Validate a successful VRD guest checkout with an AMEX card for all applicable productCodes
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Positive
  Scenario Outline: Validate a successful VRD guest checkout with a DISCOVER card for all applicable productCodes
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a guest checkout to VRD with an invalid card
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a VRD guest checkout with an invalid uxdid
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a VRD guest checkout with an invalid zip code format
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a guest checkout to VRD with an invalid product code
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
