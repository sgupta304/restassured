#############################################################################################
# This Feature file will test the the E2E functionality of Guest Checkout for DAL
...
Feature: Validate Guest Checkout DAL


  Scenario Outline: Validate a successful DAL guest checkout with a VISA card for all applicable productCodes
    ...
    Examples:
      ...
      | MOBILE      | DAL         | 0DAQE      | DLTIAD9003  | en_US  | USD      |
      | LAPTOP      | DAL         | 0DAQE      | DLTPLF0030  | en_US  | USD      |
  ...
  Scenario Outline: Validate a successful DAL guest checkout with a VISA card for all applicable productCodes
    ...
    Examples:
      | sessionType | airlineCode | tailNumber | productCode | locale | currency |
      | MOBILE      | DAL         | 0DAQE      | DLTIAD9003  | en_US  | USD      |
      | MOBILE      | DAL         | 0DAQE      | DLTIAD9104  | en_US  | USD      |
      | LAPTOP      | DAL         | 0DAQE      | DLTPLF0030  | en_US  | USD      |
      | LAPTOP      | DAL         | 0DAQE      | DLSPLT0001  | en_US  | USD      |
      | LAPTOP      | DAL         | 0DAQE      | DLTIAD9003  | en_US  | USD      |
      | LAPTOP      | DAL         | 0DAQE      | DLTIAD9104  | en_US  | USD      |

  @Positive
  Scenario Outline: Validate a successful DAL guest checkout with an MC card for all applicable productCodes
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Positive
  Scenario Outline: Validate a successful DAL guest checkout with an AMEX card for all applicable productCodes
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Positive
  Scenario Outline: Validate a successful DAL guest checkout with a DISCOVER card for all applicable productCodes
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a guest checkout to DAL with an invalid card
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a DAL guest checkout with an invalid uxdid
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a DAL guest checkout with an invalid zip code format
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
  ...
  @Negative
  Scenario Outline: Validate a guest checkout to DAL with an invalid product code
    When I generate a <sessionType> GBP SessionId with airlineCode = "<airlineCode>", tailNumber = "<tailNumber>", and deviceMacAddress = "random" and store it at "uxdid"
    And I send a CloudDao GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
    And the JSON response at "uxdId" should be "%{uxdid}"
