#############################################################################################
# This Feature file will test the the E2E functionality of message bypass for VRD.
...
Feature: Validate Message Bypass - VRD
  ...
  @HealthCheck
  Scenario: Validate a successful Message Bypass request to VRD
  ...
  @Negative
  Scenario: Validate a Message Bypass to VRD with a missing security token
    When I send a GBP POST request to generate a MOBILE session
    Then the response status should be "200"
    When I send a IFSGateway GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
  ...
  @Negative
  Scenario: Validate a Message Bypass to VRD with an invalid security token
    When I send a GBP POST request to generate a MOBILE session
    Then the response status should be "200"
    When I send a IFSGateway GET request to "/v2/token/%{uxdid}"
    Then the response status should be "200"
