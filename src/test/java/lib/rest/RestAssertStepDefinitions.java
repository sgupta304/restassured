 ...
 * @Date 05/19/2017
 */
public class RestAssertStepDefinitions extends BaseTestCase {
    ...
    public void AssertResponseStatus(int statusCode) throws Throwable {
        logger.debug("The expected response status was: " + statusCode + ", but we found: " + memory.getLastResponse().statusCode());
        assertThat(
                "The expected response status was: " + statusCode + ", but we found: " + memory.getLastResponse().statusCode(),
    ...
    public void AssertJSONHeaders() {
        ...
        assertThat(
                ...
                isOneOf("application/json;charset=UTF-8", "application/json;charset=ISO-8859-1")
        );
    }
    ...
    @Then("^the response should( not|) be \"(.*)\"$")
    public void assertResponseContent(String negate, String responseAsString){
        logger.debug("Asserting the response as a string should" + negate + " be: " + responseAsString);
        if (negate.isEmpty()){
            assertThat(
                    "Expected the response to be: " + responseAsString + ", but we found:" + memory.getLastResponse().asString(),
                    memory.getLastResponse().asString(),
                    is(responseAsString)
            );
        }
        else if (!negate.isEmpty()){
            assertThat(
                    "Expected the response to not be: " + responseAsString + ", but we found:" + memory.getLastResponse().asString(),
                    memory.getLastResponse().asString(),
                    not(is(responseAsString))
            );
        }
    }
